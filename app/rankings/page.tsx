...
type Row = { team: string; code: string; p: number; w: number; l: number; t: number; rd: number; pts: number; pct: number };
...
export default function RankingsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [ps, teams, countries] = await Promise.all([
        supabase.from("pool_standings").select("team_id,team_name,wins,losses,ties,run_differential"),
      ...
      const agg: Record<string, { team: string; code: string; w: number; l: number; t: number; rd: number }> = {};
      (ps.data as { team_id: string; team_name: string; wins: number; losses: number; ties: number; run_differential: number }[] | null)?.forEach((r) => {
        const key = r.team_id || r.team_name;
        if (!agg[key]) agg[key] = { team: r.team_name, code: teamCountry[r.team_id] || "", w: 0, l: 0, t: 0, rd: 0 };
        agg[key].w += r.wins || 0; agg[key].l += r.losses || 0; agg[key].t += r.ties || 0; agg[key].rd += r.run_differential || 0;
      });
      const list = Object.values(agg).map((a) => {
        const p = a.w + a.l + a.t;
        const pts = a.w + a.t * 0.5;                 // points: win = 1, tie = 0.5
        return { team: a.team, code: a.code, p, w: a.w, l: a.l, t: a.t, rd: a.rd, pts, pct: p ? pts / p : 0 };
      });
      list.sort((x, y) => y.pts - x.pts || y.pct - x.pct || y.rd - x.rd);
      setRows(list);
      setLoading(false);
    })();
  }, []);

  const countryOptions = useMemo(() => Array.from(new Set(rows.map((r) => r.code).filter(Boolean))).sort(), [rows]);
  ...
  return (
    <>
      <Header />
      <main>
        <section className="ev-hero" style={{ background: "radial-gradient(1100px 520px at 80% -25%, #1c3e74, var(--navy) 62%)" }}>
          <div className="wrap">
            <div className="eyebrow" style={{ color: "var(--gold)" }}>Global Ranking System</div>
            <h1>Where do you<br /><span className="ev-grad">stand in the world?</span></h1>
            <p>Ranked by points — a win is 1, a tie is ½. Filter by world, continent or country and see exactly where every team sits.</p>
          </div>
        </section>
        ...
        <section className="pad">
          <div className="wrap" style={{ maxWidth: 880 }}>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18, alignItems: "center" }}>
              <select value={continent} onChange={(e) => { setContinent(e.target.value); setCountry("all"); }} style={sel}>
            ...
            {loading ? <p style={{ color: "var(--muted)" }}>Loading rankings…</p> : filtered.length === 0 ? (
              ...
              <div style={{ border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden" }}>
                <div className="rk-row rk-head">
                  <span>#</span><span>Team</span><span>Country</span><span>W-L-T</span><span>Diff</span><span>Pct</span><span>Pts</span>
                </div>
                {filtered.map((r, i) => (
                  <div className="rk-row" key={r.team + i}>
                    ...
                    <span>{r.w}-{r.l}-{r.t}</span>
                    <span style={{ color: r.rd >= 0 ? "#138a45" : "#b3261e" }}>{r.rd >= 0 ? "+" : ""}{r.rd}</span>
                    <span>{r.pct.toFixed(3).replace(/^0/, "")}</span>
                    <span style={{ fontWeight: 800 }}>{num(r.pts)}</span>
                  </div>
                ))}
              </div>
            )}
            <p style={{ color: "var(--muted)", fontSize: 12.5, marginTop: 14 }}>
              Points = wins + ½·ties, aggregated across events; ties broken by win % then run differential. The full season points model (per Sean &amp; Marc) plugs in here.
            </p>
          </div>
        </section>
      </main>
